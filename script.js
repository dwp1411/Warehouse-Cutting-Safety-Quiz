...
function sendResults(score, totalQuestions) {
    const scriptURL = ('https://script.google.com/macros/s/AKfycbzWQ5wPVUODqI37wlsFpzKJNCcXq1aAb67sfpW3aAIR-4wJcBZR2N1lDFSkkJ5pc8pX/exec'); //  !!! YOUR WEB APP URL !!!
...
function showScore() {
    ...
    restartButton.textContent = 'Restart Quiz';  // Moved textContent here
    ...
    if (score < 10) {
        ...
        restartButton.onclick = startQuiz; // Call startQuiz directly
    }

    resultElement.appendChild(restartButton); // Append *after* setting properties
}

// Function to update the background image.
function updateBackground() {
    if (currentQuestionIndex % 2 === 0) {
        const imageIndex = Math.floor(currentQuestionIndex / 2) % 5 + 1;
        document.body.style.backgroundImage = `url('images/background${imageIndex}.jpg')`;
    }
}
